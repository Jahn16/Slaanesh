...
config_dictionary = {
    ...
    "platforms": ["PC", "PlayStation", "Xbox", "Nintendo", "VR"],
    "playing": ["playing", "on hold"],
    "played positive": ["completed", "mastered"],
    "played negative": ["discarded"],
    "played": [],
...
class TabsConfig(BaseModel):
    type_playing: str = "cards"
    type_played: str = "table"
    type_backlog: str = "cards"
    type_wishlist: str = "table"
    filter_playing: bool = True
    filter_played: bool = True
    filter_backlog: bool = False
    filter_wishlist: bool = False
...
class IGDBConfig(BaseModel):
    ...
    auth_token: str = ""
    token_timestamp: str = "2000-01-01 12:12:12"
...
class Settings(BaseModel):
    ...
    platforms: list[str] = ["PC", "PlayStation", "Xbox", "Nintendo", "VR"]
    playing: list[str] = ["playing", "on hold"]
    played_positive: list[str] = Field(
        default=["completed", "mastered"], alias="played positive"
    )
    played_negative: list[str] = Field(default=["discarded"], alias="played negative")
    played: list[str] = []
    backlog: list[str] = ["backlog", "waiting"]
    wishlist: list[str] = ["wishlist"]
    unplayed: list[str] = []
...
def load_config():
    ...
    for section in config:
        ...
        for key in config[section]:
            ...
            if config[section][key].isnumeric():
                config_dictionary[section][key] = config.getint(
                    section, key, fallback=config_dictionary[section][key]
                )
            # boolean
            elif config[section][key] == "True" or config[section][key] == "False":
                config_dictionary[section][key] = config.getboolean(
                    section, key, fallback=config_dictionary[section][key]
                )
    ...
    config_dictionary["played"] = (
        config_dictionary["played positive"] + config_dictionary["played negative"]
    )
    config_dictionary["unplayed"] = (
        config_dictionary["backlog"]
        + config_dictionary["wishlist"]
        + config_dictionary["playing"]
    )
        ...
        if update[0] is not None:
            ...
            if update[1] in config[update[0]] and update[2] is not None:
                if type(update[2]) is bool:
                    ...
                    config[update[0]][update[1]] = config_dictionary[update[0]][
                        update[1]
                    ] = str(update[2])
